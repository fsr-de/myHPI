...
class MailingList(object):
    ...
    def _raise_nomember(self, email):
        raise exceptions.NoMemberException(
            "{} is not a member address of {}".format(email, self.fqdn_listname)
        )
    ...
    def add_member_silently(self, email):
        ...
        self.list.subscribe(
            email, pre_verified=True, pre_confirmed=True, send_welcome_message=False
        )
    ...
    def pending_subscriptions(self, request_type="subscription"):
        ...
        if settings.DISABLE_GOODBYE_MESSAGES:
            ...
            response, answer = self.conn.rest_call(
                "{}?{}".format(path, urllib.parse.urlencode(get_params), None, "GET")
            )
            ...
            return {
                r["token"]: r["email"] for r in self.list.get_requests(token_owner="subscriber")
            }
    ...
    def confirm_subscription(self, token):
        self._wrap_subscription_exception(lambda: self.list.accept_request(token), token)

    def cancel_pending_subscription(self, token):
        self._wrap_subscription_exception(
            lambda: self.list.moderate_request(token, "discard"), token
        )
    ...
    def demote_from_owner(self, email):
        ...
        if not self.list.is_owner(email):
            raise exceptions.NoMemberException(
                "{} is not an owner address of {}".format(email, self.fqdn_listname)
            )
    ...
    def get_roster(self):
        memberships = {
            e["email"]: (False, self._is_a_blocked_action(e.get("moderation_action")))
            for e in self._raw_get_roster("member")
        }
        memberships.update(
            {
                ...
                email: (True, memberships[email][1])
                for email in (e["email"] for e in self._raw_get_roster("owner"))
    ...
    def inject_message(self, sender_address, subject, message, other_headers=None):
        ...
        other_headers_str = "\n".join(
            ["%s: %s" % (k.capitalize(), str(v)) for k, v in other_headers.items()]
        )
        if other_headers_str:
            other_headers_str += "\n"
        raw_text = ("From: {}\n" "To: {}\n" "Subject: {}\n" + other_headers_str + "\n" "{}").format(
            sender_address, self.fqdn_listname, subject, message
        )
